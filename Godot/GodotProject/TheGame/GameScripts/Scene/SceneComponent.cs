...
namespace GodotGameFramework.Scene
{
	public partial class SceneComponent : GameFrameworkComponent
	{
		...
		private readonly Dictionary<string, Node> m_SceneInstance = new Dictionary<string, Node>();
		private Node m_CurrentScene = default(Node);
		public Node CurrentActiveScene => m_CurrentScene;
		...
		public override void OnInit()
		{
			...
			m_CurrentScene = GetTree().CurrentScene;
		...
		public override void OnEnter()
		{
			...
			{
				m_SceneManager.SetResourceManager(baseComponent.EditorResourceManager);
			}
			else
			{
		...
		public bool HasScene(string sceneAssetName)
		{
			...
			if (!sceneAssetName.StartsWith(GameFolderConstant.ReadOnlyPath, StringComparison.Ordinal) || !sceneAssetName.EndsWith(".tscn", StringComparison.Ordinal))
			{
				Log.Error("Scene asset name '{0}' is invalid.", sceneAssetName);
				return false;
			}
		...
		public void UnloadScene(string sceneAssetName, object userData)
		{
			...
			if (m_SceneInstance.ContainsKey(sceneAssetName))
			{
				Node scene = m_SceneInstance[sceneAssetName];
				if (m_CurrentScene == scene) m_CurrentScene = null;
				scene.QueueFree();
				m_SceneInstance.Remove(sceneAssetName);
			}
		}
		...
		private void RefreshSceneOrder()
		{
			if (m_SceneOrder.Count > 0)
			{
				...
				if (maxSceneName == null)
				{
					SetActiveScene(m_CurrentScene);
					return;
				}

				string currentSceneName = GetSceneName(maxSceneName);
				if (!string.IsNullOrEmpty(currentSceneName))
				{
					GF.Resource.LoadAssetAsync(maxSceneName, typeof(PackedScene), s =>
					{
						if (s != null)
						{
							Node sence = (s as PackedScene).Instantiate();
							sence.Name = currentSceneName;
							SetActiveScene(sence);
							AddChild(sence);
							m_SceneInstance.TryAdd(maxSceneName, sence);
						}
					});
				}
				else
				{
					Log.Warning("Scene node for '{0}' not found in loaded scene nodes.", maxSceneName);
				}
			}
			else
			{
		...
		private void SetActiveScene(Node activeScene)
		{
			if (activeScene == null)
			{
				return;
			...
			Node lastActiveScene = GetTree().CurrentScene;
