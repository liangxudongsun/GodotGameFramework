...
//         {
//             // 超时自动消失（不扣分）
//             GD.Print($"  [RedBlock] 超时消失 (Entity {Entity?.Id})");
...
//     {
//         GD.Print($"  [RedBlock] 点击! {m_Score} 分 (Entity {Entity?.Id})");

//         // 触发分数变化事件
//         if (GF.Event != null)
//         {
//             GF.Event.Fire(Entity, ScoreChangedEventArgs.Create(m_Score));
//             GF.Event.Fire(Entity, BlockClickedEventArgs.Create(Entity.Id, 2, m_Score));
//         }
